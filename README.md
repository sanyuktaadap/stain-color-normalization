...
2. **Create CSV**: Create a CSV file containing the names of the 120 selected images. The CSV should have a column labeled `slide_id` and look like this:
...
- Use the script `main.py` to plot segmentation maps. The script requires the following arguments:
...
- The segmentations produced can then be used for running the **Normalization Pipeline**.
...
- The normalization pipeline consists of three main scripts:
  1. `1-Produce_Image_Stain_Vectors_and_Optical_Density.py`
  2. `2-Aggregate_Stain_Vectors_and_Histograms.py`
  3. `3-Normalize_Image.py`

- Use the script `run.sh` to execute the pipeline. Update the paths for the following variables in `run.sh`:
  ...
  - **Gray_Level_Labels_Directory**: Path to a CSV file containing the following 3 columns:
...
- **Image Files**: Can be in `.jpg`, `.png`, or `.tif` formats (non-pyramidal).
